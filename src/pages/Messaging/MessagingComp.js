...
//      let senderId = Conversation?.members[0]?.senderId
//      let receiverId = Conversation?.members[0]?.receiverId
//      //console.log("mndsdjn", Userr)

//      console.log("andanbdancsjncs", Conversation)
//      console.log("userData", userData?._id)



//      console.log("filter", Conversation?.members?.filter((m) => m[0]?.receiverId?._id !== currentUser?._id))
...
//                         <Typography className={classes.imgs} component="img" src={senderId?._id == userData?._id? receiverId?.profilePic: senderId?.profilePic}></Typography>
//                     </Box>
//                     <Box className={classes.widthright}>
//                         <Box>
//                             <Typography variant="h6" color="#949494">{senderId?._id == userData?._id? receiverId?.userName: senderId?.userName}</Typography>
...
const MessagingComp = ({ Conversation, currentUser, query="" }) => {
    const classes = useStyle();
    const [user, setUser] = useState(null);
    const [{ userData }] = useContext(UserContext);
    ...
    return (
        <>
            ...
            <Link to="/messagingExpend" className={classes.listitem}>
                ...
                {user?.userName?.toLowerCase()?.includes(query?.toLowerCase()) ?<Box className={classes.listitem2}>
                    <Box className={classes.widthleft}>
